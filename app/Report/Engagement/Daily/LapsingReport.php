...
class LapsingReport
{
  ...
  {
    $this->fname = "Lapsing-Report-".date('Y-m-d') . uniqid();
    $this->process();
  }
      ...
      $this->data = DB::select(
          DB::raw("SELECT DS1.*
                ...
                (SELECT users.id, users.name,users.account_no, users.mobile_no, users.email, users.balance, users.status, users.created_at FROM users
                        ...
                        WHERE transactions.created_at < DATE_SUB(CURDATE(), INTERVAL 30 DAY)
                ...
                SELECT users.id, users.name,users.account_no, users.mobile_no, users.email, users.balance, users.status, users.created_at FROM users
                        ...
                        WHERE transactions.created_at < DATE_SUB(CURDATE(), INTERVAL 30 DAY)
                        GROUP BY users.id, users.account_no, users.mobile_no, users.email, users.balance, users.status, users.created_at
                        HAVING COUNT(transactions.id) > 1) AS DS1
                ...
                (SELECT users.id, users.name,users.account_no, users.mobile_no, users.email, users.balance, users.status, users.created_at FROM users
                        ...
                        WHERE
                            ...
                            transactions.created_at < DATE_SUB(CURDATE(), INTERVAL 1 DAY) AND
